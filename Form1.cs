...
namespace FileExplorer
{
    public partial class Form1 : Form
    {
        ...
        private void btnFiltrele_Click(object sender, EventArgs e)
        {
                ...
                {
                    toplam += Convert.ToDouble(listView1.Items[i].SubItems[1].Text);
                }
                string sonuc = "";
                if (toplam < 1024)
                {
                ...
                else if (toplam < 1048576)
                {
                    sonuc = (toplam / 1024).ToString("N2") + " KB";
                }
                else
                {
                    sonuc = (toplam / 1024 / 1024).ToString("N2") + " Mb";
                }
                toolStripStatusLabel1.Text = string.Format("{0} adet dosya, {1} boyut", adet, sonuc);
                textChange = true;
                cmbFiltre.Text = "*" + seciliUzanti;
                textChange = false;
            //string yenisecilen = cmbFiltre.SelectedIndex.ToString();
            //if (yenisecilen == seciliUzanti)
            //{
            //    DosyalariDoldur(treeView1.SelectedNode.Tag.ToString());
            //}
        }
        ...
        public void DosyalariDoldur(string path)
        {
            ...
            foreach (FileInfo fiDosya in dosyalar)
            {
                ...
                string s = uzanti + " " + Convert.ToString(sayac);
                if (!cmbFiltre.Items.Contains(s))
                {
                    sayac = 1;
                    s = uzanti + " " + Convert.ToString(sayac);
                    cmbFiltre.Items.Add(s);
                }
                    a++;
                    cmbFiltre.Items.Remove(uzanti);
                    cmbFiltre.Items.Add(uzanti + a);
            }
